...
void JNIEXPORT DLIB_JNI_METHOD(jniNativeClassInit)(JNIEnv* _env, jclass _this) {
  ...
  if(gVisionDetRetOffsets.confidence == NULL || gVisionDetRetOffsets.left == NULL || gVisionDetRetOffsets.top == NULL 
      || gVisionDetRetOffsets.right == NULL || gVisionDetRetOffsets.bottom== NULL ) {
      LOG(FATAL) << "Can't Find one of the fields of  \"VisionDetRet\".\n";
  }
  
  gVisionDetRetOffsets.addLandmark = _env->GetMethodID(detRetClass, "addLandmark", "(II)Z" );
  if(gVisionDetRetOffsets.addLandmark == NULL) {
      LOG(FATAL) << "Can't Find Method \"addLandmark(int,int)\".\n";
  }

  LOG(INFO) << "VisionDetRet init Success";
  LOG(INFO) << "JniNativeClassInit Success";
}
  ...
  if (gDLibHOGFaceDetectorPtr) {
    ...
    if (faceShapeMap.find(index) != faceShapeMap.end()) {
      ...
      for (int i = 0 ; i != shape.num_parts(); i++) {
        ...
        // Call addLandmark on the FaceLandmarks field
